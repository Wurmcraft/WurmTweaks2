...
public class ScriptExecutor {
  ...
  private static NonBlockingHashMap<String, FunctionWrapper>[] sortedFunctions =
      new NonBlockingHashMap[2];
            ...
            () -> {
              ...
              for (File script : ScriptChecker.getScriptsToRun()) {
                if (script.exists()) {
                  ...
                  thread.setName(
                      "WurmScript Pre-Worker (" + script.getName().replaceAll(".ws", "") + ")");
              ...
              for (File script : ScriptChecker.getScriptsToRun()) {
                ...
                thread.setName(
                    "WurmScript Worker (" + script.getName().replaceAll(".ws", "") + ")");
                threads.add(thread);
                thread.start();
              }
        ...
        () -> {
          ...
          if (bindings.size() == 0) {
            WurmTweaks.logger.error(
                "Unable to load any WurmScript Functions, Stopping Thread, "
                    + sortedFunctions[1].size()
                    + " "
                    + functions.size());
          } else {
            try {
