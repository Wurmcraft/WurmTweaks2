...
public class ActuallyAdditions {
  ...
  public void init() {
    ...
    if (ScriptExecutor.reload) {
      ...
      scriptLensConversion
          .forEach(ActuallyAdditionsAPI.RECONSTRUCTOR_LENS_CONVERSION_RECIPES::remove);
  ...
  public void finalizeSupport() {
    scriptCrusher.forEach(crusher -> ActuallyAdditionsAPI
        .addCrusherRecipe(crusher.getInput(), crusher.getOutputOne(), crusher.getOutputTwo(),
    ...
    scriptTreasure.forEach(loot -> ActuallyAdditionsAPI
    ...
    scriptEmpowerer.forEach(recipe -> ActuallyAdditionsAPI
        .addEmpowererRecipe(recipe.getInput(), recipe.getOutput(), recipe.getStandOne(),
            ...
            recipe.getEnergyPerStand(), recipe.getTime(), recipe.getParticleColors()));
    scriptCompost.forEach(recipe -> ActuallyAdditionsAPI
        .addCompostRecipe(recipe.getInput(), recipe.getInputDisplay(), recipe.getOutput(),
            recipe.getOutputDisplay()));
    scriptStone
        .forEach(stone -> ActuallyAdditionsAPI.addMiningLensStoneOre(stone.name, stone.itemWeight));
  ...
  @ScriptFunction(modid = "actuallyadditions", inputFormat = "ItemStack Integer", typeData = "Scrap", type = FunctionType.Linked)
  ...
  public void addTreasureChest(Converter converter, String[] line) {
    scriptTreasure.add(
        new TreasureChestLoot((ItemStack) converter.convert(line[0]), Integer.parseInt(line[1]),
            ((ItemStack) converter.convert(line[0])).getCount(), Integer.parseInt(line[2])));
  }
  ...
  public void addEmpowerer(Converter converter, String[] line) {
    scriptEmpowerer.add(new EmpowererRecipe((ItemStack) converter.convert(line[1]),
        ...
        Integer.parseInt(line[7]), new float[]{0xFF2F21, 158F / 255F, 43F / 255F, 39F / 255F}));
  }
  ...
  public void addComposter(Converter converter, String[] line) {
    scriptCompost.add(new CompostRecipe((ItemStack) converter.convert(line[2]),
  ...
  @ScriptFunction(modid = "actuallyadditions", inputFormat = "String OreDictionary Integer", typeData = "Laser", type = FunctionType.Linked)
  public void addAALaser(Converter converter, String[] line) {
    if (line[0].equalsIgnoreCase("stone")) {
      scriptStone.add(new WeightedOre(line[1].replaceAll(StackSettings.START.getData(), "")
          .replaceAll(StackSettings.END.getData(), ""), Integer.parseInt(line[2])));
    } else if (line[0].equalsIgnoreCase("nether")) {
      scriptNether.add(new WeightedOre(line[1].replaceAll(StackSettings.START.getData(), "")
          .replaceAll(StackSettings.END.getData(), ""), Integer.parseInt(line[2])));
    }
  }
}
