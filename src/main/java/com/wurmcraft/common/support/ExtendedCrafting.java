...
public class ExtendedCrafting {
  ...
  public void init() {
    if (scriptTableRecipes == null || scriptEnderShaped == null || scriptCompression == null
    ...
    if (ConfigHandler.removeAllRecipes) {
      scriptTableRecipes
          .forEach(output -> TableRecipeManager.getInstance().removeRecipes((ItemStack) output[0]));
  ...
  public void finalize() {
    scriptTableRecipes.forEach(recipe -> TableRecipeManager.getInstance()
        .addShaped((int) recipe[1], (ItemStack) recipe[0],
            Arrays.copyOfRange(recipe, 1, recipe.length)));
    scriptEnderShaped.forEach(recipe -> EnderCrafterRecipeManager.getInstance()
        .addShaped((ItemStack) recipe[0], recipe[1]));
    scriptEnderShapeless.forEach(recipe -> EnderCrafterRecipeManager.getInstance()
        .addShapeless((ItemStack) recipe[0], recipe[1]));
    for (CompressorRecipe recipe : scriptCompression) {
      CompressorRecipeManager.getInstance()
          .addRecipe(recipe.getOutput(), recipe.getInput(), recipe.getInputCount(),
              recipe.getCatalyst(), recipe.consumeCatalyst(), recipe.getPowerCost(),
              recipe.getPowerRate());
    }
  }
  ...
  public void addShapedTable(Converter converter, String[] line) {
    scriptTableRecipes.add(new Object[]{converter.convert(line[0]), Integer.parseInt(line[1]),
  ...
  public void addShapelessTable(Converter converter, String[] line) {
    scriptTableRecipes.add(new Object[]{converter.convert(line[0]),
