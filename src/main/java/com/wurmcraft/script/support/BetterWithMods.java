...
public class BetterWithMods extends SupportHelper {
	...
	public void finishSupport () {
		for (Object[] recipe : cauldron)
			BWRegistry.CAULDRON.addUnstokedRecipe ((List <Ingredient>) recipe[0],(List <ItemStack>) recipe[1]);
		for (Object[] recipe : crucible)
			BWRegistry.CRUCIBLE.addUnstokedRecipe ((List <Ingredient>) recipe[0],(List <ItemStack>) recipe[1]);
		for (Object[] recipe : sCauldron)
			BWRegistry.CAULDRON.addStokedRecipe ((List <Ingredient>) recipe[0],(List <ItemStack>) recipe[1]);
		for (Object[] recipe : sCrucible)
			BWRegistry.CRUCIBLE.addStokedRecipe ((List <Ingredient>) recipe[0],(List <ItemStack>) recipe[1]);
		for (Object[] recipe : mill)
			BWRegistry.MILLSTONE.addMillRecipe ((List <Ingredient>) recipe[0],(List <ItemStack>) recipe[1]);
		for (Object[] recipe : saw)
			BWRegistry.WOOD_SAW.addRecipe ((ItemStack) recipe[0],(ItemStack) recipe[1]);
		for (Object[] recipe : anvil)
			AnvilCraftingManager.ANVIL_CRAFTING.add ((ShapedAnvilRecipe) recipe[0]);
		for (Object[] recipe : heat)
			BWMHeatRegistry.addHeatSource ((Block) recipe[0],(int) recipe[1]);
	}
	...
	public void addCauldron (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length >= 2,"addCauldron('<output> <input>...')");
	...
	public void addStokedCauldron (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length >= 2,"addStokedCauldron('<output> <input>...')");
		isValid (helper,input[0]);
		List <Ingredient> inputStacks = new ArrayList <> ();
		for (int index = 1; index < input.length; index++) {
			isValid (helper,input[index]);
	...
	public void addCrucible (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length >= 2,"addCrucible('<output> <input>...')");
		isValid (helper,input[0]);
		List <Ingredient> inputStacks = new ArrayList <> ();
		for (int index = 1; index < input.length; index++) {
			isValid (helper,input[index]);
	...
	public void addStokedCrucible (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length >= 2,"addStokedCrucible('<output> <input>...')");
		isValid (helper,input[0]);
		List <Ingredient> inputStacks = new ArrayList <> ();
		for (int index = 1; index < input.length; index++) {
			isValid (helper,input[index]);
	...
	public void addMill (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length >= 2,"addMill('<output> <input>...')");
		List <Ingredient> inputStacks = new ArrayList <> ();
		for (int index = 1; index < input.length; index++) {
			isValid (helper,input[index]);
			inputStacks.add (convertIngredient (helper,input[index]));
	...
	public void addWoodSaw (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 2,"addSaw('<output> <input>')");
	...
	public void addAnvil (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length > 4,"addAnvil(<output> <style> <format>')");
		isValid (helper,input[0]);
	...
	public void addBlockHeat (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 2,"addBlcokHeat(<block> <heat>')");
		isValid (helper,input[0]);
