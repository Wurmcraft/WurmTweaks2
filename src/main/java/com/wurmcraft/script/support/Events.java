...
public class Events extends SupportHelper {
 ...
 public void addTooltip(StackHelper helper, String line) {
  String[] input = validateFormat(line, line.split(" ").length >= 2, "addTooltip('<output> <tooltip-lines>...')");
  isValid(helper, input[0]);
  List<String> tips = new ArrayList<>();
  for (int index = 1; index < input.length; index++)
   tips.add(input[index].replaceAll("&", "\u00A7").replaceAll(StackSettings.SPACE.getFormatting(), " "));
 ...
 public void convertPickup(StackHelper helper, String line) {
  String[] input = validateFormat(line, line.split(" ").length == 2, "convertPickup('<from> <to>')");
