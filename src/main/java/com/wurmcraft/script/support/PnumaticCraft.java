...
public class PnumaticCraft extends SupportHelper {
	...
	public void addAssemblyDrill (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 2,"addAssemblyDrill('<output> <input')");
	...
	public void addAssemblyLaser (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 2,"addAssemblyLaser('<output> <input')");
		isValid (helper,input[0],input[1]);
	...
	public void addPressureChamber (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length >= 3,"addPressureChamber('<output> <pressure> <input>...')");
	...
	public void addDefaultAmadron (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 2,"addDefaultAmadron('<output> <input>')");
		isValid (helper,input[0],input[1]);
