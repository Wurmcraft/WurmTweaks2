...
public class EnvironmentalTech extends SupportHelper {
	...
	public void addBotanicMiner (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 4,"addBotanicMiner('<stack> <color> <weight> <tier>'");
		isValid (helper,input[0]);
		isValid (Types.INTEGER,helper,input[1],input[2]);
		botanic.add (new Object[] {convertInteger (input[3]),new WeightedItemStack (convertStack (helper,input[0]),convertInteger (input[2])),input[1]});
	}

	@ScriptFunction
	public void addResourceMiner (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 4,"addResourceMiner('<stack> <color> <weight> <tier>'");
		isValid (helper,input[0]);
		isValid (Types.INTEGER,helper,input[1],input[2]);
		res.add (new Object[] {convertInteger (input[3]),new WeightedItemStack (convertStack (helper,input[0]),convertInteger (input[2])),input[1]});
	}

	@ScriptFunction
	public void addOreMiner (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 4,"addOreMiner('<stack> <color> <weight> <tier>'");
		isValid (helper,input[0]);
		isValid (Types.INTEGER,helper,input[1],input[2]);
		ore.add (new Object[] {convertInteger (input[3]),new WeightedItemStack (convertStack (helper,input[0]),convertInteger (input[2])),input[1]});
	}
}
