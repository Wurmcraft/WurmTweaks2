...
public class Thaumcraft extends SupportHelper {
	...
	private static AspectList getAspectList (String[] aspect) {
		...
		for (String spect : aspect)
			if (spect.startsWith (StackSettings.FRONT.toString () + StackSettings.ASPECT.toString ())) {
				Aspect aspe = getAspect (spect.substring (spect.indexOf (StackSettings.FRONT.toString ().charAt (0)) + 1,spect.indexOf (StackSettings.BACK.toString ().charAt (0))));
				int amount = Integer.parseInt (spect.substring (spect.indexOf (StackSettings.FRONT.toString () + StackSettings.ASPECT.toString ()) + 2,spect.indexOf (StackSettings.STACK_SIZE.toString ())));
	...
	public void addLootBagItem (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 2,"addLootBagItem('<output> <bag>')");
	...
	public void addCrucible (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length >= 4,"addCrucible('<output> <researchKey> <input> <aspects>...')");
	...
	public void addInfusion (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length >= 7,"addInfusion(<output> <research> <centerStack> <instability> <items>... <aspects...'");
		isValid (helper,input[0],input[2]);
		isValid (Types.INTEGER, helper,input[3]);
		List <ItemStack> items = new ArrayList <> ();
		for (int index = 4; index < input.length; index++)
			if (!input[index].startsWith (StackSettings.FRONT.toString () + StackSettings.ASPECT.toString ())) {
