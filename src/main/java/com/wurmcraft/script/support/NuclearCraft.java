...
public class NuclearCraft extends SupportHelper {
	...
	public void addManufactory (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 3,"addManufactory('<output> <input> <time>')");
	...
	public void addIsotopeSeparator (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 4,"addIsotopeSeparator('<output> <output2> <input> <time')");
	...
	public void addPressurizer (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 3,"addPressurizer('<output> <input> <time>')");
		isValid (helper,input[0],input[1]);
		isValid (Types.INTEGER,helper,input[2]);
