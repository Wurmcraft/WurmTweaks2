...
public class ActuallyAdditions extends SupportHelper {
	...
	public void addEmpowerer (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 8,"addEmpowerer('<output> <inputCenter> <input> <input2> <input3> <input4> <energyPerStand> <time>')");
	...
	public void addReconstructor (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 3,"addReconstructor('<output> <input> <energy>')");
	...
	public void addAACrusher (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 4,"addAACrusher('<output> <input> <secOutput> <secOutput%>')");
	...
	public void addMiningLensStone (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 2,"addMiningLensStone('<oreDict> <weight>')");
	...
	public void addMiningLensNether (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 2,"addMiningLensNether('<oreDict> <weight>')");
		isValid (Types.STRING,helper,input[0]);
		isValid (Types.INTEGER,helper,input[1]);
	...
	public void addComposter (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 2 || line.split (" ").length == 4,"addComposter('<output> <input> | <outputDisplay> <inputDisplay>')");
		isValid (helper,input[0],input[1]);
