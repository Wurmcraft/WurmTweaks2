...
public class AstralSorcery extends SupportHelper {
	...
	public void addBasicInfusion (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 2,"addBasicInfusion('<output> <input>')");
	...
	public void addSlowInfusion (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 2,"addSlowInfusion('<output> <input>')");
		isValid (helper,input[0],input[1]);
