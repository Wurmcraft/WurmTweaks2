...
public class TConstruct extends SupportHelper {
	...
	private List <Object[]> melting = Collections.synchronizedList (new ArrayList <> ());
	...
	public void finishSupport () {
		...
		for (Object[] recipe : melting)
			TinkerRegistry.registerMelting ((ItemStack) recipe[0],(Fluid) recipe[1],(int) recipe[2]);
	...
	public void addCasting (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 3,"addCasting('<output> <cast> <*fluid>')");
	...
	public void addBasin (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 3,"addBasin('<output> <cast> <*fluid>')");
	...
	public void addAlloy (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length >= 2,"addAlloy('<*output> <*input>...')");
	...
	public void addDrying (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 3,"addDrying('<output> <input> <time>')");
		isValid (helper,input[0],input[1]);
	...
	public void addFuel (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 2,"addFuel(<*fluid> <time>')");
		isValid (Types.FLUIDSTACK, helper,input[0]);
	...
	public void addMelting (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 3,"addMelting('<*output> <input> <temp>')");
		isValid (Types.FLUIDSTACK, helper,input[0]);
		isValid (helper,input[1]);
		isValid (Types.INTEGER,helper,input[2]);
		FluidStack fluidStack = convertFluidStack (helper,input[0]);
		melting.add (new Object[] {new MeltingRecipe (RecipeMatch.of (convertStack (helper,input[1]),fluidStack.amount),fluidStack,convertInteger (input[2]))});
	}

	@ScriptFunction
	public void addEntityMelting (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 2,"addEntityMelting('<*output> entityName')");
	...
	public void handleMelting (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 3,"handleMelting('<*fluid> <ingot> <block>')");
		isValid (Types.FLUIDSTACK, helper,input[0]);
		...
		if (convertStack (helper,input[2]) != ItemStack.EMPTY) {
			melting.add (new Object[] {convertStack (helper,input[2]),fluid,BLOCK});
			basin.add (new Object[] {convertStack (helper,input[2]),null,fluid,BLOCK});
		}
		if (convertStack (helper,input[1]) != ItemStack.EMPTY) {
			melting.add (new Object[] {convertStack (helper,input[1]),fluid,INGOT});
	...
	public void handleMaterialParts (StackHelper helper,String line) {
		String[] input = validate (line,line.split (" ").length == 2,"handleMaterialParts('<materialName> <*fluid>')");
		...
		for (IToolPart toolPart : TinkerRegistry.getToolParts ())
			if (toolPart instanceof MaterialItem && toolPart.canBeCasted ()) {
				...
				melting.add (new Object[] {stack,fluid,normalize (toolPart.getCost ())});
